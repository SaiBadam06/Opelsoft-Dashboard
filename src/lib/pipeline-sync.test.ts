...
describe("stageAfterRemoval (recompute when a submission is deleted)", () => {
  it("drops a gated stage that lost all backing back to new", () => {
    expect(stageAfterRemoval("placed", [], [])).toBe("new");
    expect(stageAfterRemoval("interview_r2", [], [])).toBe("new");
  });

  it("moves a gated stage to what the remaining submissions support", () => {
    expect(stageAfterRemoval("placed", ["submitted"], [])).toBe("submitted");
    expect(stageAfterRemoval("placed", ["rejected"], [])).toBe("rejected");
  });

  it("keeps a gated stage still supported by a remaining interview", () => {
    expect(
      stageAfterRemoval("interview_r2", [], [{ round: "R2", result: "scheduled" }]),
    ).toBe("interview_r2");
  });

  it("leaves a free pre-submission stage untouched", () => {
    expect(stageAfterRemoval("screening", [], [])).toBe("screening");
    expect(stageAfterRemoval("matched", [], [])).toBe("matched");
  });
});
