...
export async function ensurePlacementFromSubmission(
  ...
  try {
    ...
    console.error("[pipeline-sync] ensurePlacementFromSubmission failed", e);
  }
}
...
export async function writeBackStageToSubmission(
  ...
  try {
    ...
    console.error("[pipeline-sync] writeBackStageToSubmission failed", e);
  }
}
...
export async function syncCandidateStage(
  ...
  try {
    ...
    console.error("[pipeline-sync] syncCandidateStage failed", e);
  }
}

// When a submission is deleted, recompute the candidate's stage so a gated stage
// (submitted…placed, rejected, hold) can't linger without backing. Reads the
// remaining submissions/interviews and applies stageAfterRemoval. Best-effort.
export async function recomputeStageAfterSubmissionRemoval(
  supabase: ServerClient,
  candidateId: string,
  actorId: string,
): Promise<void> {
  try {
    ...
    const current = cand.pipeline_stage as PipelineStage;
    const next = stageAfterRemoval(current, statuses, interviews);
    ...
    console.error("[pipeline-sync] recomputeStageAfterSubmissionRemoval failed", e);
  }
}
